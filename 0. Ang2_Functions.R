...
# For the first 3 days, TRUE if the first 2 days' scores are none-zero and the 3rd day's score is 2/3
no_zero_in_first_2_and_2_3_in_day_3 <- function(row) {
  # Split the row by commas and convert to numeric values
  values <- as.numeric(unlist(strsplit(row, ",")))
  
  # Return TRUE if the first two values are > 0 and the third value is 2 or 3
  return(!is.na(values[1]) && values[1] > 0 &&
           !is.na(values[2]) && values[2] > 0 &&
           !is.na(values[3]) && (values[3] == 2 || values[3] == 3))
}